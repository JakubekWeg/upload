...
function init(app) {
    ...
    const FILES_SORTING_METHODS = {
        'name-asc': [(o1, o2) => o1.name > o2.name ? 1 : -1, 'Sort by name A-Z'],
        'name-desc': [(o1, o2) => o1.name < o2.name ? 1 : -1, 'Sort by name Z-A'],
        'upload-asc': [(o1, o2) => o2.createdDate - o1.createdDate, 'Sort by upload time, newest first'],
        'upload-desc': [(o1, o2) => o1.createdDate - o2.createdDate, 'Sort by upload time, oldest first'],
        'size-desc': [(o1, o2) => o2.size - o1.size, 'Sort by size bigger first'],
        'size-asc': [(o1, o2) => o1.size - o2.size, 'Sort by size smaller first'],
    };
    app.get('/files', (req, res) => global_1.withUser(req, res, async (user) => {
        const files = await Promise.all(user.filesList.map(e => database_1.INSTANCE.getFileInfo(e)));
        let sortingMethod = FILES_SORTING_METHODS[req.query.sort];
        if (!sortingMethod) {
    ...
    app.get('/files/:fileId/:action?', async (req, res, next) => {
        ...
        const file = await database_1.INSTANCE.getFileInfo(fileId);
        ...
        if (file) {
            const fileAccessSuccessCallback = (isOwner) => {
                try {
            ...
            global_1.withUser(req, res, (user) => {
    ...
    app.get('/files/:fileId/change/visibility/:value', (req, res) => global_1.withUser(req, res, async (user) => {
        const file = await database_1.INSTANCE.getFileInfo(req.params.fileId);
        if (!file || user.uid !== file.ownerUid)
    ...
    app.post('/files/:fileId/delete', (req, res) => global_1.withUser(req, res, async (user) => {
        const file = await database_1.INSTANCE.getFileInfo(req.params.fileId);
        if (!file || user.uid !== file.ownerUid)
    ...
    /// -------------- upload -----------------
    app.get('/upload', (req, res) => global_1.withUser(req, res, (user) => {
        res.render('upload', {
    ...
    app.post('/upload', (req, res) => global_1.withUser(req, res, (user) => {
        global_1.handleFileUpload(req, res, user, (file) => {
