...
export function init(app: Express) {
    ...
    app.get('/files', (req, res) => withUser(req, res, (user) => {
        const files = user.filesList
    ...
    const SUPPORTED_INLINE_PREVIREWS = ['text', 'image', 'video', 'audio']

    app.get('/files/:fileId/:action?', (req, res, next) => {
        ...
        if (file) {
                ...
                try {
                    switch (req.params.action) {
                        case 'download':
                            ...
                            res.type(file.type)
                            res.sendFile(file.fid, { root: UPLOADS_FOLDER }, (e) => { if (e) fileAccessErrorCallback })
                            break;
                        case 'view':
                        ...
                        default:
                            const showPreview = SUPPORTED_INLINE_PREVIREWS.includes(file.type.split('/')[0])
            ...
            withUser(req, res, (user) => {
                const isOwner = file.owner.uid === user.uid
    ...
    app.get('/files/:fileId/change/visibility/:value', (req, res) => withUser(req, res, (user) => {
        const file = user.fileById(req.params.fileId)
        if (!file) return res.status(403).render('error', {
    ...
    app.post('/files/:fileId/delete', (req, res) => withUser(req, res, (user) => {
        const file = user.fileById(req.params.fileId)
        ...
        unlink(UPLOADS_FOLDER + '/' + file.fid, (err) => {
            if (err) {
            ...
            user.unregisterFile(file)
            db.unregisterFile(file.fid)
            db.save()
    ...
    app.post('/upload', (req, res) => withUser(req, res, (user) => {
        handleFileUpload(req, res, () => user, (filename, size, fid) => {
            res.set('File-Id', fid)
