...
export const withAdminUser = (
  req: Request,
  res: Response,
  then: UserCallback
) =>
  withUser(req, res, (user) => {
    if (user.isAdmin) then(user);
    else
...
export const handleFileUpload = function (
  req: Request,
  res: Response,
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      if (!(user instanceof User) && !!user) user = await user(fields);
      ...
      const file = await db.createFileInfo(
        files.file.path,
        fileId,
