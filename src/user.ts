...
export class User {
  ...
  public set password(newHashedPassword: string) {
    ...
    db._updateUserInfo(this.uid, { password: newHashedPassword }).then(
      (r) => r
    );
  }
  ...
  destroyActiveSession(sid: string) {
    ...
    if (index >= 0) this.activeSessions.splice(index, 1);
  }
  ...
  async deleteMe() {
    if (this.uid === "root") throw new Error("Can't delete root user!");
