...
import { DEFAULT_ROOT_PASSWORD, HTTP_SERVER_PORT, STATIC_ASSETS_MAX_AGE } from './configuration'
...
(async () => {
	...
	app.post('/login', POST_DATA_HANDLER, (req, res) => withUser(req, res,
		() => res.redirect('/'), async () => {
			try {
				const user = await db.signInUserByPassword(req.body.login, req.body.password)
				if (req.session) {
